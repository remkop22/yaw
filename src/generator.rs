use crate::analysis::{Table, Action};
	...
	NT: Hash + Eq + Copy,
{
	...
	for (state, action) in table.actions() {
		for (nt, action) in action {
			...
			unique_terminals.insert(String::from(str_enum.replace(enum_var.as_str(), "&'static str")));
	...
	for term in unique_terminals {
		tt_enum.new_variant(format!("{}", term).as_str());
	}
	...
	for (state, nt, action) in unique_actions {
		let action_str =  match action {
			...
			Action::Reduce(rule) => format!("self.reduce({})", rules.iter().position(|r| r == &rule).unwrap()),
			Action::Accept => "self.accept()".into(),
			Action::Error => "self.error()".into()
		};
		
		action_fn.line(format!("({}, TokenType::{:?}) => {},", state, nt, action_str));
	}
